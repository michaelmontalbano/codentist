...
# === Слоты ===
SLOTS_FILE = "available_slots_by_doctor.xlsx"
if os.path.exists(SLOTS_FILE):
    slots_df = pd.read_excel(SLOTS_FILE)
    slots_df["datetime"] = pd.to_datetime(slots_df["datetime"])
else:
    # create an empty dataframe so the tools don't error if the file is missing
    slots_df = pd.DataFrame(columns=["doctor", "datetime", "weekday"])
...
def add_patient_to_db(data: dict) -> str:
    ...
    if "Today's_Date" in PATIENT_COLUMNS and not new_entry.get("Today's_Date"):
